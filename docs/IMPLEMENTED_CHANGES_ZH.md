...
| 当前本地源码 | `821f33b` | 比远程 `main` 超前 5 个提交，尚未重新打包 |
| Python 包版本 | `0.7.2b2` | 尚未随最新两个提交再次升版 |
| 当前自动测试 | 115 项 | 全部通过 |

当前本地源码包含的五个未进入远程稳定版提交：
...
该功能已通过自动测试，但还没有进入新的安装包。安装了 v0.7.1 或 v0.7.2 Beta 2 的设备仍然使用旧登录行为。
