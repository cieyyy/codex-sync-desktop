...
首次启动会打开四步配置向导，帮助普通用户完成网络检查、GitHub 注册/登录、私有仓库创建和本机配置。中国大陆用户可以在向导中填写本机 HTTP 代理并先测试 GitHub 连接。
...
5. 向导检测 Git、GitHub CLI 和登录状态后，点击“自动创建并完成”。
6. 在源设备点击“导出并推送”；在其他设备安装软件并使用同一 GitHub 账号连接该私有仓库。

日常使用可以直接点击“一键同步”：未选择其他来源设备时只上传本机；选择来源设备后会依次拉取、导入合并、修复侧栏并上传本机。包含导入时必须先退出 Codex、ChatGPT 和 Codex++。

面向完全没有 Git/GitHub CLI 的用户，当前向导会提供官方下载入口；安装完成后回到向导点击“重新检测”。详见 [小白首次配置手册](docs/FIRST_RUN_GUIDE.md)。
